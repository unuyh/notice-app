...
try:
    ...
    if content_groups:
        ...
        # 1. 제목과 버튼이 들어갈 컨테이너 (고정 폭 설정)
        # 제목 스타일: flex-shrink로 제목이 줄어들게 하고, overflow/text-overflow로 글자 짤림 방지
        # 버튼 스타일: flex-shrink: 0으로 사이즈 고정
        js_content = content_to_copy.replace("\n", "\\n").replace("'", "\\'")
        
        container_html = f"""
        <div style="display: flex; align-items: center; justify-content: space-between; width: 100%; height: 60px; background-color: #262730; padding: 10px; border-radius: 5px;">
            <h3 style="margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; padding-right: 15px;">
                📌 {selected_option} 내용
            </h3>
            <button onclick="navigator.clipboard.writeText('{js_content}'); alert('복사 완료!');" 
                    style="flex-shrink: 0; width: 80px; height: 35px; cursor: pointer; background-color: #ff4b4b; color: white; border: none; border-radius: 5px; font-weight: bold;">
                📋 Copy
            </button>
        </div>
        """
